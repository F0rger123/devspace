...
export interface RecentActivityItem {
  ...
  projectId: string;
  projectName: string;
  type: 'project_edit' | 'issue' | 'idea' | 'roadmap' | 'note' | 'commit' | 'push' | 'pull_request' | 'conversation_action';
  title: string;
  summary?: string;
  timestamp: number;
  source: 'workspace' | 'github' | 'aether' | 'notes';
...
class AetherActiveProjectContextService {
  ...
  public async fetchGitHubStateForRepo(repoFullName: string, branch: string = 'main'): Promise<boolean> {
    ...
    try {
      // 1. Fetch commits
      const commitRes = await fetch('/api/github/pull', {
        method: 'POST',
      ...
      try {
        const prRes = await fetch('/api/github/pulls', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
   ...
   */
  public getRecentWorkReport(options?: { targetProjectName?: string; timeFilter?: 'today' | 'yesterday' | 'this_week' | 'last_month' | 'recent'; allProjects?: any[] }): RecentWorkReport {
    const facts: string[] = [];
    ...
    if (inferredThemes.length > 0) {
      ...
      inferences.push(`You've been organizing project goals and refining workspace features in ${primaryProjectNames[0] || activeProjName}.`);
    }
    ...
    const themesText = inferredThemes.length > 0
    ...
    const spokenText = `You've mainly been working on ${spokenProject}. ${themesText}`;
