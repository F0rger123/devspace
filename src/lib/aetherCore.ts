  ...
  private initializeDefaultState() {
    ...
    const skillsList: SkillDefinition[] = [
      ...
      },
      {
        ...
        description: 'Inspect Cloudflare Pages & Workers deployments, manage build queues, pause duplicate previews, and verify production edge workers.',
        category: 'workspace',
        version: '1.5.0',
        enabled: true,
        authStatus: 'connected',
        health: 'healthy',
        capabilities: ['Deployment Health Inspection', 'Duplicate Preview Cancellation', 'Build Queue Management', 'Production Edge Verification'],
        permissionsRequired: [
          {
            id: 'perm-cloudflare-api',
            skillId: 'skill-cloudflare',
            scopeName: 'pages:read_write',
            granted: true,
            whyNeeded: 'Monitors Cloudflare build queues and pauses stale or duplicate preview deployments.',
            dataAccess: 'Cloudflare Pages project list, deployment status logs, build queues.',
            frequency: 'Every 15 minutes',
            isStored: true,
            lastAccessed: Date.now() - 300000,
          },
        ],
        config: { autoCancelDuplicatePreviews: true, maxConcurrentPreviews: 3 },
        lastAccessTime: Date.now() - 300000,
      },
      {
        id: 'skill-repo-ops',
        name: 'Repository Operations Center',
        description: 'Analyze repository branches, audit merged vs stale branches, calculate branch health metrics, and execute safe automated cleanups with explicit approval.',
        category: 'git_dev',
        version: '2.4.0',
        enabled: true,
        authStatus: 'connected',
        health: 'healthy',
        capabilities: ['Branch Health Analysis', 'Stale Branch Archival', 'Merged Branch Cleanup', 'Conflict Risk Audit'],
        permissionsRequired: [
          {
            id: 'perm-repo-cleanup',
            skillId: 'skill-repo-ops',
            scopeName: 'repo:admin_read',
            granted: true,
            whyNeeded: 'Scans repository tree for stale, duplicate, or merged branches eligible for safe archival.',
            dataAccess: 'Branch ref logs, commit ages, pull request status.',
            frequency: 'Daily',
            isStored: true,
            lastAccessed: Date.now() - 180000,
          },
        ],
        config: { autoArchiveAfterDays: 30, autoDeleteMergedAfterDays: 14, requireExplicitConfirmation: true },
        lastAccessTime: Date.now() - 180000,
      },
    ];
